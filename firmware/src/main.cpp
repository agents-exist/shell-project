...
// Advertise as "Shell-XXXX" (last two BT MAC bytes) so multiple sticks
// in one room are distinguishable in the desktop picker. Name persists in
// btName for the BLUETOOTH info page.
static char btName[16] = "Shell";
static void startBt() {
  uint8_t mac[6] = {0};
  esp_read_mac(mac, ESP_MAC_BT);
  snprintf(btName, sizeof(btName), "Shell-%02X%02X", mac[4], mac[5]);
  bleInit(btName);
}
...
void loop() {
  ...
  // overlays, no prompt, no live agent data, and the RTC has been set
